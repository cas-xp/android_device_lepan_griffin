...
{
LOGE("direction request. devices %x,  result %x\n", handle->devices, 
	 handle->devices & AudioSystem::DEVICE_OUT_ALL);
...
LOGV("%s", __FUNCTION__);
    ...
    if (devices & 0x0000FFFF){
        ...
        if (devices & (AudioSystem::DEVICE_OUT_SPEAKER |
            ...
            control.set("HeadsetR Mixer AudioR2", (unsigned int)1); // on
            control.set("HeadsetL Mixer AudioL2", (unsigned int)1); // on
			control.set("PredriveL Mixer AudioL2", 1);
			control.set("PredriveR Mixer AudioR2", 1);
            control.set("PreDriv Playback Volume", 18); //Headset Volume
            control.set("DAC2 Analog Playback Volume", 18); //Headset Volume
            control.set("DAC2 Digital Fine Playback Volume", 44); //Headset Volume
            control.set("Digtal mute", 'off'); //Digital mute
			control.set("Earpiece Playback Volume", (unsigned int)0);
			control.set("Headset Playback Volume",3);
        } else {
        ...
        if (devices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) {
            ...
            control.set("HandsfreeR Switch", (unsigned int)0);
            control.set("HandsfreeL Switch", (unsigned int)0);
        } else {
    ...
    /* for input devices */
    if (devices >> 16) {
		control.set("TX1 Digital Capture Volume", 10);
        control.set("Digtal mute", 'off'); //Digital mute
