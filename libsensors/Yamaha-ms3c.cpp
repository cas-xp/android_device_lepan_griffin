...
Ms3cSensor::Ms3cSensor()
: SensorBase(KXTF9_DEVICE_NAME, "orientation"),
      mEnabled(0),
      mPendingMask(0),
      mInputReader(32)
{
...
int Ms3cSensor::enable(int32_t handle, int en)
{
    int err = 0;
...
int Ms3cSensor::setDelay(int32_t handle, int64_t ns)
{
    int err = 0;
...
int Ms3cSensor::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
...
void Ms3cSensor::processEvent(int code, int value)
{
    switch (code) {
